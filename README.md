...
🟢 **Image and video both production-quality (2026-05-23).** Image pipelines (t2i, image_edit, x2t_image) reproduce the bf16 PyTorch reference. Video pipelines (t2v, video_edit, x2t_video) landed two production fixes: **Phase 5j position-ID fix** (`latent_pos_base=0`, closed [issue #2](https://github.com/xocialize/lance-mlx/issues/2)) and **Phase 5m CFG-renorm fix** ([v0.5.2](https://github.com/xocialize/lance-mlx/releases/tag/v0.5.2-phase5m-cfg-renorm), `cfg_renorm_type="channel"`). **Production envelope is now n_lat ≤ 16,128** (256² through 768²×25f, 480×704×17f) — verified across two diagnostic prompts (panda surfing, bus + Big Ben). Phase 5m root cause: the legacy `"global"` CFG-renorm cap computed a single scalar L2 over the full velocity tensor, so at ~2× n_lat the same cap silently over-suppressed high-frequency detail. `"channel"` clamps per-channel without dragging the aggregate. The n_lat → quality relationship inside the envelope is stochastic seed × scale, not a monotonic degradation curve. The n_lat ≥ ~30k regime (768²×49f, 480×848×121f) is **pending manual verification** — pre-Phase-5m it collapsed to noise, post-Phase-5m it has not yet been re-tested at this scale. Tracked at [issue #1](https://github.com/xocialize/lance-mlx/issues/1).
...
| **t2v (text → video)** | ✅ **Production. Photoreal/CGI-quality output** at n_lat ≤ 16,128 (256–768² × ≤25f; 480×704×17f; 640²×17f) after Phase 5j position-ID fix + Phase 5m CFG-renorm fix. Verified across two diagnostic prompts (panda surfing, bus + Big Ben). ⚠️ Pending manual verification at n_lat ≥ ~30k (768²×49f, 480×848×121f) — [issue #1](https://github.com/xocialize/lance-mlx/issues/1). |
